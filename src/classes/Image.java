...
public class Image {
	...
	public void changeDirLogo(int width, int height) {
		
		//tratar colisão com a borda lateral direita
		if(this.posX  + this.width > width) {
			this.posX = width - this.width;
			this.sound.play();
		}
		
		//tratar colisão com a borda lateral esquerda
		if(this.posX < 0) {
			this.posX = 0;
			this.sound.play();
		}
		
		//tratar colisão com a borda inferior
		if(this.posY + this.height > height) {
			this.posY = height - this.height;
			this.sound.play();
		}
		
		//tratar colisão com a borda superior
		if(this.posY  < 0) {
			this.posY = 0;
			this.sound.play();
		}
